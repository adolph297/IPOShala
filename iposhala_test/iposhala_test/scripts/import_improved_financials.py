...
# Config
BASE_DIR = os.getcwd()
JSON_FILE = os.path.join(BASE_DIR, "batch_6_improved_financials.json")
...
def run():
    ...
    for symbol, content in data.items():
        if not content or not content.get("audited_financials"): 
            continue
        ...
        for item in financials:
            # Use desc preferably, fallback to label/type
            label = item.get("desc") or item.get("label") or item.get("type")
            
            doc = {
                "desc": label,
                "url": item.get("url"),
                "source": item.get("source", "selenium_v2"),
        ...
        update_fields["nse_company.extracted_improved"] = financials
