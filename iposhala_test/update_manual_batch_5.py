    ...
    for symbol, website in updates.items():
        if website:
            ...
            if result.modified_count > 0:
                ...
                # Check if it was already updated or symbol not found
                doc = collection.find_one({"symbol": symbol})
                if doc:
                    if doc.get("company_website") == website:
                        print(f"Skipped {symbol}: Already up to date")
                    else:
                        print(f"Skipped {symbol}: Document found but no update needed?")
                else:
                    print(f"Warning: Symbol {symbol} not found in DB")
        else:
            print(f"Skipping {symbol}: No website found")

    print(f"Total companies updated in this batch: {count}")
